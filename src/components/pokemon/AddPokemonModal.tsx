...
interface AddPokemonModalProps {
  ...
  onAddPokemon: (pokemon: CaughtPokemon, moveIds: PokemonMoves['id'][]) => Promise<void>;
...
export function AddPokemonModal({ isOpen, setIsOpen, onAddPokemon, availablePokemon }: AddPokemonModalProps) {
  ...
  const loadPokemonDetails = async (pokemonId: string): Promise<void> => {
  ...
  const handlePokemonChange = (pokemonId: string) => {
    void loadPokemonDetails(pokemonId)
  }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (Object.keys(newErrors).length === 0 && selectedPokemon) {
      ...
      void onAddPokemon(caughtPokemon, selectedMoves.map(m => m.id))
      handleClose()
    }
  }
          ...
          <div className={selectedPokemon ? 'md:grid md:grid-cols-2 md:gap-8' : ''}>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                >
                  <option value="">
                    {isFetchingData ? 'Loading Pokemon details...' : 'Choose a Pokemon...'}
                  </option>
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2 max-h-64 overflow-y-auto border rounded-md p-3">
                    {selectedPokemon.availableMoves.map((move, index) => {
                      const isSelected = Boolean(selectedMoves.find(m => m.name === move.name))
                      const isDisabled = !isSelected && selectedMoves.length >= MAX_MOVES
                      return (
                        <button
                          key={`${move.name}-${index}`}
                          type="button"
                    ...
                    <div className="bg-gray-50 rounded-lg p-4">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {selectedMoves.map((move, index) => (
                          <PokemonMove key={index} move={{...move, type: DEFAULT_MOVE_TYPE}} />
                        ))}
                      </div>
                    </div>
                  )}
